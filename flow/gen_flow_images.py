...
def gen_images(gen_label_only=True):
    ...
    label_files = open('/home/cory/project/yolo2-pytorch/train_data/kitti/kitti_train_labels.txt')
    ...
    img_pairs = open('img_pairs.txt', 'w')
    ...
    for i in range(0, len(image_abs_paths)):
        ...
        # for flownet 2.0
        flo_id = out_path[out_path.find('/') + 1:].replace('/', '_')
        img_pairs.write(curr_img_path + ' ' + prev_img_path + ' ../output/' + flo_id + '.flo\n')
